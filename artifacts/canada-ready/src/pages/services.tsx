...
export default function Services() {
  ...
  return (
    <div style={{ width: "100%" }}>
      <section style={{ padding: "70px 5% 36px", background: `linear-gradient(135deg, ${BG}, #edfdfb)`, textAlign: "center" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: "6px", background: "rgba(129,216,208,0.12)", border: "1px solid rgba(129,216,208,0.25)", color: TDK, padding: "6px 14px", borderRadius: "14px", fontSize: "11px", fontWeight: 700, letterSpacing: "1.2px", textTransform: "uppercase", marginBottom: "14px" }}>🧾 {isAr ? "كل الخدمات" : "ALL SERVICES"}</div>
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(32px,4vw,56px)", fontWeight: 800, color: TDK, marginBottom: "12px" }}>{isAr ? <>كل الدعم الذي تحتاجه للنجاح في <span style={{ color: GOLD }}>كندا</span></> : <>All the Support You Need to Succeed in <span style={{ color: GOLD }}>Canada</span></>}</h1>
        <p style={{ fontSize: "17px", color: MID, maxWidth: "760px", margin: "0 auto", lineHeight: 1.75 }}>{isAr ? "هذه الصفحة تجمع كل أفكار الخدمات والدعم في مكان واحد — بشكل واضح، منظم، ومتوافق مع الجوال." : "This page gathers all service ideas in one place — clear, organized, and mobile-friendly."}</p>
      </section>
      <section style={{ padding: "62px 5%", background: "#fff" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: "16px" }}>
            {sections.map((section) => (
              <div key={section.title_en} style={{ background: BG, border: "1px solid rgba(129,216,208,0.18)", borderRadius: "18px", padding: "22px" }}>
                <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "22px", fontWeight: 800, color: TDK, marginBottom: "14px" }}>{isAr ? section.title_ar : section.title_en}</h2>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                  {(isAr ? section.items_ar : section.items_en).map((item) => (
                    <span key={item} style={{ background: "#fff", border: "1px solid rgba(129,216,208,0.18)", color: MID, padding: "8px 11px", borderRadius: "999px", fontSize: "12px", fontWeight: 600, lineHeight: 1.4 }}>{item}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      <section style={{ background: TDK, padding: "68px 5%", textAlign: "center" }}>
